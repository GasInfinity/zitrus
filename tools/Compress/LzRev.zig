...
pub fn run(args: LzRev, io: std.Io, arena: std.mem.Allocator) !u8 {
    ...
    const data = try input_reader.interface.allocRemaining(arena, .unlimited);
    ...
    if (args.decompress) |_| {
        ...
        const decompressed_len = zitrus.compress.lzrev.len(data);

        if (args.verbose) |_| log.info("lzrev compressed {} vs decompressed {}", .{ data.len, decompressed_len });
    ...
    try output_writer.interface.writeAll(compressed);
    try output_writer.interface.flush();
    return 0;
}
