...
pub const Id = enum(u16) {
    ...
    pub fn fromRegister(comptime base: *gpu.Registers.Internal, register: *anyopaque) Id {
        std.debug.assert(@intFromPtr(register) >= @intFromPtr(base) and @intFromPtr(register) < (@intFromPtr(base) + @sizeOf(gpu.Registers.Internal))); // invalid internal register, pointer is not within the valid range
...
pub const Queue = struct {
    ...
    pub fn add(queue: *Queue, comptime base: *gpu.Registers.Internal, register: anytype, value: std.meta.Child(@TypeOf(register))) void {
        return queue.addMasked(base, register, value, 0xF);
    }

    pub fn addMasked(queue: *Queue, comptime base: *gpu.Registers.Internal, register: anytype, value: std.meta.Child(@TypeOf(register)), mask: u4) void {
    ...
    fn IncrementalWritesTuple(comptime base: *gpu.Registers.Internal, comptime registers: anytype) type {
    ...
    pub fn addIncremental(queue: *Queue, comptime base: *gpu.Registers.Internal, comptime registers: anytype, values: IncrementalWritesTuple(base, registers)) void {
        return queue.addIncrementalMasked(base, registers, values, 0b1111);
    }

    pub fn addIncrementalMasked(queue: *Queue, comptime base: *gpu.Registers.Internal, comptime registers: anytype, values: IncrementalWritesTuple(base, registers), mask: u4) void {
