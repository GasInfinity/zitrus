...
pub const Token = struct {
    ...
    pub const mnemonics = std.StaticStringMap(Tag).initComptime(.{
        .{ "add", .mnemonic_add },
        .{ "dp3", .mnemonic_dp3 },
        .{ "dp4", .mnemonic_dp4 },
        .{ "dph", .mnemonic_dph },
    ...
    pub fn getMnemonic(str: []const u8) ?Tag {
    ...
    pub fn next(tok: *Tokenizer) Token {
        ...
        state: switch (State.start) {
            .start => switch (tok.buffer[tok.index]) {
                ...
                '.' => {
                    result.tag = .directive;
                    continue :state .directive;
                },
            ...
            .directive => {
                ...
                switch (tok.buffer[tok.index]) {
                    'A'...'Z', 'a'...'z', '_', '.', '0'...'9' => continue :state .directive,
                    else => {},
                } 
            },
            .identifier => {
                ...
                switch (tok.buffer[tok.index]) {
                    'A'...'Z', 'a'...'z', '_', '.', '0'...'9' => continue :state .identifier,
                        ...
                        if(Token.getMnemonic(tok.buffer[result.loc.start..tok.index])) |tag| {
                            result.tag = tag;
                        }
                    },
                }
...
test "tokenize directive" {
    try testTokenize(".directive.with an.identifier.parameter 0b10", &.{ .directive, .identifier, .number_literal });
}
